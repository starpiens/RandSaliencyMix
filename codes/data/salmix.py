...
def ramdomly_selected_threshold_saliency_bbox(img, lam):
    size = img.size()
    width = size[1]
    height = size[2]
    cut_rat = np.sqrt(1. - lam)
    ...
    while True:
        x_index = np.random.choice(len(x_y_indices[0]))
        y_index = np.random.choice(len(x_y_indices[1]))
        x = x_y_indices[0][x_index]
        y = x_y_indices[1][y_index]
        ...
        #    continue
        sal_mean = saliency_map[bbx1:bbx2, bby1:bby2].mean()
        if sal_mean > threshold:
